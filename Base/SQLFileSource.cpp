...
int SQLFileSource::createSQLTable(SQLHSTMT hstmt, const char *table) {
	char query[8192];
	char *p;
...
int SQLFileSource::prepareReadQuery() {
	char *p = query;
	bool isFirstColumn = true;
	int i;
	IRowManipulator *row = getRowManipulator();
...
int SQLFileSource::prepareWriteQuery() {
	char *ptr = query;
	int curCol;
	bool isFirstColumn = true;
	IRowManipulator *row = getRowManipulator();
...
int SQLFileSource::prepareWriteRowQuery() {
	bool isFirstColumn = true;
...
int SQLFileSource::prepareReadRowQuery(SQLHSTMT hstmt) {
	IRowManipulator *row = getRowManipulator();
	void *buffer;
