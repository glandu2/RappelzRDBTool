...
public:
	...
	inline void* DLLCALLCONV getValuePtrByName(const char* columnName) {
		return getValuePtr(getColumnIndex(columnName));
	}
	void* DLLCALLCONV getValuePtr(int colPos);

	int DLLCALLCONV getDataCount(int colPos);  // use the value saved in TYPE_VARCHAR_SIZE to get TYPE_VARCHAR_STR size
	                                           // else return the same as getMaxDataCount()
	void DLLCALLCONV setDataCount(int colPos, int dataCount);

	inline void** DLLCALLCONV getCurrentRow() { return row; }
	inline void DLLCALLCONV setCurrentRow(void** row) {
		this->row = row;
		initRow(true);
	}
	...
	inline int DLLCALLCONV getMaxDataCount(int colPos) {
		return GET_VAR_LNGTH(columnsTypeOrdered.fieldsInfo[colPos].dataSize);
	}
	inline int DLLCALLCONV getDataIndex(int colPos) {
		return GET_VAR_INDEX(columnsTypeOrdered.fieldsInfo[colPos].dataSize);
	}
	inline int DLLCALLCONV getType(int colPos) { return GET_TYPE(columnsTypeOrdered.fieldsInfo[colPos].dataType); }
	inline int DLLCALLCONV getIgnoreType(int colPos) {
		return GET_IGNORETYPE(columnsTypeOrdered.fieldsInfo[colPos].dataType);
	}
	...
	inline const char* DLLCALLCONV getColumnName(int colPos) {
		return columnsTypeOrdered.fieldsInfo[colPos].columnName;
	}
#ifndef NDEBUG
	inline int DLLCALLCONV getColumnOriginalIndex(int i) {
		if(!order)
			return i;
		if(!(i < order->size)) {
			*((char*) 0) = 0;
	...
	void DLLCALLCONV setDataBitArray(const char* columnName,
	                                 char* array,
	                                 int size);  // value can contains at most 8 bits (more is not supported atm)
