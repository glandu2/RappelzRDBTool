...
void EDATABASEDLL DLLCALLCONV convertData(eDataFormat dst,
	...
	if(mode == DCT_Write && dst == DF_RDB) {
		...
		row->setDataInt32(
		    "item_use_flag2",
		    row->getDataBit("flag_wear") | (row->getDataBit("flag_storage") << 1) |
		        (row->getDataBit("flag_enhance") << 2) | (row->getDataBit("flag_use") << 3) |
		        (row->getDataBit("flag_card") << 4) | (row->getDataBit("flag_socket") << 5) |
		        (row->getDataBit("flag_duplicate") << 6) | (row->getDataBit("flag_target_use") << 7) |
		        (row->getDataBit("flag_warp") << 8) | (row->getDataBit("flag_trade") << 9) |
		        (row->getDataBit("flag_sell") << 10) | (row->getDataBit("flag_quest") << 11) |
		        (row->getDataBit("flag_overweight") << 12) | (row->getDataBit("flag_cashitem") << 13) |
		        (row->getDataBit("flag_riding") << 14) | (row->getDataBit("flag_drop") << 15) |
		        (row->getDataBit("flag_move") << 16) | (row->getDataBit("flag_quest_distribute") << 17) |
		        (row->getDataBit("flag_sit") << 18) | (row->getDataBit("flag_raid") << 19) |
		        (row->getDataBit("flag_secroute") << 20) | (row->getDataBit("flag_eventmap") << 21) |
		        (row->getDataBit("flag_huntaholic") << 22) | (row->getDataBit("flag_huntaholic_useable_only") << 23) |
		        (row->getDataBit("flag_deathmatch") << 24) | (row->getDataBit("flag_deathmatch_useable_only") << 25));
